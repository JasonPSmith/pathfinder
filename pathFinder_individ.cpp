...
struct parameters_t {
    ...
    parameters_t(int argc, char** argv){
        ...
        auto it_vertex = named_arguments.find("vertex");
        ...
        if (it_vertex == named_arguments.end() || it_vertices == named_arguments.end() || it_threads == named_arguments.end() || it_size == named_arguments.end() || it_pathdist == named_arguments.end() || it_out == named_arguments.end() || it_indices == named_arguments.end() || it_indptr == named_arguments.end()) {
            std::cerr << "ERROR: an input is missing" << std::endl; exit(-1);
        }

        number_of_vertices = atol(it_size->second);
        start_vertex = atol(it_vertex->second);
        ...
        start_vertex_loc = array_index(start_vertex,vertices);
        if (start_vertex_loc == -1) { std::cerr << "ERROR: vertex not in vertices" << std::endl; exit(-1); }
...
void extend_cycle(vertex_t vertex, std::vector<vertex_t>& prefix,
                       int thread_id, parameters_t& params) {
    ...
    if(loc != -1 && prefix.size() <= params.path_dist[params.start_vertex_loc][loc]){
        for (auto i : current_prefix) params.outstreams[thread_id] << i << " ";
        params.outstreams[thread_id] << std::endl;
    }

    if (current_prefix.size() <= params.maxpath[params.start_vertex_loc]){
        ...
        for(auto iter = params.indptr[vertex]; iter != params.indptr[vertex+1]; ++iter){
            if(!array_contains(params.indices[iter],current_prefix)){
                extend_cycle(params.indices[iter], current_prefix, thread_id, params);
            }
        }
    }
}
...
void start_count(int thread_id, parameters_t& params) {
    ...
    for(auto index = params.indptr[params.start_vertex]+thread_id; index < params.indptr[params.start_vertex+1]; index += params.parallel_threads){
        first_vertices.push_back(params.indices[index]);
    }

    std::vector<vertex_t> prefix; //vertices in path so far
    prefix.push_back(params.start_vertex);
    ...
    for (auto index : first_vertices){
        extend_cycle(index, prefix, thread_id, params);
        {
            LockIO lock;
            params.vertices_completed++;
            std::cout << index << " : thread " << thread_id << " : ";
            std::cout << params.vertices_completed << "/" << params.indptr[params.start_vertex+1]-params.indptr[params.start_vertex] << std::endl;
        }
    }
}
